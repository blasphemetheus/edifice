defmodule Edifice.Generative.STARFlow do
  @moduledoc """
  STARFlow: Scaled Transformer Autoregressive Flow.

  Extends TarFlow with a deep-shallow block architecture: one deep transformer
  block captures global structure while remaining shallow blocks refine details
  cheaply. Direction alternation (reversing sequence order between blocks)
  ensures universal approximation.

  ```
  Input [batch, num_patches, patch_dim]
        |
  +-------------------------------------------+
  | Deep Block (deep_layers transformer layers)|
  |   Causal attention + FFN                   |
  |   -> Predict (scale, shift) per position   |
  |   -> Affine: y = x * exp(s) + t            |
  +-------------------------------------------+
        | reverse sequence
  +-------------------------------------------+
  | Shallow Block 1 (shallow_layers layers)    |
  |   Same affine coupling structure           |
  +-------------------------------------------+
        | reverse sequence
  +-------------------------------------------+
  | Shallow Block 2 ...                        |
  ...
  Returns `{encoder, decoder}` tuple where encoder maps data to latent
  space (with log-determinant for training) and decoder maps latent to data.

  ## Usage

      {encoder, decoder} = STARFlow.build(
        input_size: 64,
        num_flows: 4,
        deep_layers: 8,
        shallow_layers: 2,
  ...
  - Gu et al., "STARFlow: Scaling Latent Normalizing Flows" (Apple, NeurIPS 2025)
  """

  @default_input_size 64
  @default_num_flows 4
  @default_deep_layers 8
  @default_shallow_layers 2
  ...
  @type build_opt ::
          ...
          | {:deep_layers, pos_integer()}
          | {:shallow_layers, pos_integer()}
          | {:hidden_size, pos_integer()}
          | {:num_heads, pos_integer()}
          | {:dropout, float()}

  @doc """
  Build a STARFlow model.

  Returns `{encoder, decoder}` where:
  - `encoder` maps data to latent space (produces log-det for training)
  - `decoder` maps latent to data space (for generation)

  ## Options

    - `:input_size` - Dimension of each patch/token (default: 64)
    - `:num_flows` - Total number of flow blocks (default: 4)
    - `:deep_layers` - Transformer layers in the first (deep) block (default: 8)
    - `:shallow_layers` - Transformer layers in remaining blocks (default: 2)
  ...
  def build(opts \\ []) do
    ...
    deep_layers = Keyword.get(opts, :deep_layers, @default_deep_layers)
    shallow_layers = Keyword.get(opts, :shallow_layers, @default_shallow_layers)
  ...
  defp build_encoder(
    ...
    # Apply flow steps: first is deep, rest are shallow
    # With direction alternation between blocks
    {x, log_det} =
      Enum.reduce(0..(num_flows - 1), {x, nil}, fn i, {curr, acc_logdet} ->
        num_block_layers = if i == 0, do: deep_layers, else: shallow_layers

        # Direction alternation: reverse sequence for odd-indexed blocks
        curr =
          if rem(i, 2) == 1 do
            Axon.nx(curr, &reverse_sequence/1, name: "enc_reverse_#{i}")
          else
            curr
          end
  ...
  defp build_decoder(
    ...
    x =
      Enum.reduce((num_flows - 1)..0//-1, x, fn i, curr ->
        ...
        # Direction alternation (same pattern as encoder)
        curr =
          if rem(i, 2) == 1 do
            Axon.nx(curr, &reverse_sequence/1, name: "dec_reverse_#{i}")
          else
            curr
          end
  ...
  # Build a transformer block with causal masking for flow parameter prediction
  defp build_flow_transformer(input, hidden_size, num_heads, num_layers, dropout, name) do
    head_dim = div(hidden_size, num_heads)

    Enum.reduce(0..(num_layers - 1), input, fn j, acc ->
      layer_name = "#{name}_layer_#{j}"

      q = Axon.dense(acc, hidden_size, name: "#{layer_name}_q")
      k = Axon.dense(acc, hidden_size, name: "#{layer_name}_k")
      v = Axon.dense(acc, hidden_size, name: "#{layer_name}_v")

      attn_out =
        Axon.layer(
          &causal_attention_impl/4,
          [q, k, v],
          name: "#{layer_name}_attn",
          num_heads: num_heads,
          head_dim: head_dim,
          op_name: :causal_attention
        )

      attn_out = Axon.dense(attn_out, hidden_size, name: "#{layer_name}_attn_proj")
      x = Axon.add(acc, attn_out)
      x = Axon.layer_norm(x, name: "#{layer_name}_ln1")

      ffn =
        x
        |> Axon.dense(hidden_size * 4, name: "#{layer_name}_ffn1")
        |> Axon.activation(:silu)
        |> Axon.dense(hidden_size, name: "#{layer_name}_ffn2")

      ffn = if dropout > 0, do: Axon.dropout(ffn, rate: dropout), else: ffn

      x = Axon.add(x, ffn)
      Axon.layer_norm(x, name: "#{layer_name}_ln2")
    end)
  end

  defp reverse_sequence(x) do
    Nx.reverse(x, axes: [1])
  end
  ...
  defp build_affine_forward(input, affine_params, input_size, name) do
    transformed =
      Axon.layer(
        fn x, params, _opts ->
          {s, t} = split_affine(params, input_size)
          s = Nx.clip(s, -5.0, 5.0)
    ...
    log_det =
      Axon.layer(
        fn _x, params, _opts ->
          {s, _t} = split_affine(params, input_size)
          s = Nx.clip(s, -5.0, 5.0)
          Nx.sum(s, axes: [-1])
        end,
        [input, affine_params],
  ...
  defp split_affine(params, input_size) do
