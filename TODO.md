...
- [ ] **CUDA Kernel Fusion (P9) — Multi-Layer Block Scan Fusion** — Keep hidden state in registers across consecutive layers, fusing LayerNorm + GEMV + scan + residual into one kernel per timestep. Extends existing MinGRU/MinLSTM block pattern to additional architectures with scalar/small state.
  ...
  - [ ] **Phase 4 — Benchmarks & Architecture Wiring** — `bench/block_scan_sweep.exs`, wire Griffin/DeepResLSTM `build_fused_block` paths.
