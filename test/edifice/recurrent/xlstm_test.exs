defmodule Edifice.Recurrent.XLSTMTest do
  ...
  describe "build/1 with embed_size == hidden_size" do
  ...
  describe "param_count/1" do
    test "returns a positive integer for sLSTM" do
      count = XLSTM.param_count(embed_size: 64, hidden_size: 32, num_layers: 2, variant: :slstm)
    ...
    test "returns a positive integer for mixed" do
      count = XLSTM.param_count(embed_size: 64, hidden_size: 32, num_layers: 4, variant: :mixed)
      assert is_integer(count)
      assert count > 0
    end
  end
