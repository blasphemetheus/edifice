defmodule Edifice.Vision.FocalNetTest do
  use ExUnit.Case, async: true

  alias Edifice.Vision.FocalNet
  ...
  describe "build/1" do
    test "returns an Axon model" do
      model = FocalNet.build(@base_opts)
      assert %Axon{} = model
    end
    ...
    test "works with different focal levels" do
      output = build_and_predict(Keyword.put(@base_opts, :focal_levels, 2))
      assert Nx.shape(output) == {@batch, @hidden_size}
    end

    test "works with single focal level" do
      output = build_and_predict(Keyword.put(@base_opts, :focal_levels, 1))
      assert Nx.shape(output) == {@batch, @hidden_size}
    end
  end
  ...
  describe "output_size/1" do
    test "returns hidden_size by default" do
      assert FocalNet.output_size(@base_opts) == @hidden_size
    end

    test "returns num_classes when set" do
      assert FocalNet.output_size(Keyword.put(@base_opts, :num_classes, 10)) == 10
    end
  end
end
